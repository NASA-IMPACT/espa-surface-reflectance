...
unsigned char *mem_buffer[N_mem_buffers];
size_t mem_buffer_size[N_mem_buffers];
size_t mem_buffer_pos[N_mem_buffers];
...
void levels(char *, int, int);
int PDStimes(int time_range, int p1, int p2, int time_unit, int *n_hrs_forecast);
...
int ASCII_TCA_PDS_date(int frcst_hours, unsigned char *pds, int v_time, char date[], char *what);
int addd_time(int *year, int *month, int *day, int *hour, int dtime, int unit);
int verf_time(unsigned char *pds, int *year, int *month, int *day, int *hour);

int read_grib_array(FILE *input, char *what, char *where, int *n_rows, int *n_cols, float **narray)
/*
!C*****************************************************************************
...
*/
{
    ...
    enum { DUMP_ALL, DUMP_RECORD, DUMP_POSITION, DUMP_LIST, INVENTORY } mode = INVENTORY;
    ...
    for (;;) {
        ...
        levels(level, PDS_KPDS6(pds), PDS_KPDS7(pds));
        if ((!strcmp(k5toa(pds), what)) && (!strcmp(level, where))) {
            /*
                    ...
                    PDStimes(PDS_TimeRange(pds),PDS_P1(pds),PDS_P2(pds),
                        ...
                        printf("\n");
            */
            if ((*narray = (float *)realloc(*narray, sizeof(float) * nxny)) == NULL) {
                return (-1);
            }
            ...
            BDS_unpack(temp_array, bds + 11, BMS_bitmap(bms), BDS_NumBits(bds), nxny, temp * BDS_RefValue(bds),
...
int read_grib_date(FILE *input, char *what, char *where, char *date, int *frcst_val)
/*
!C*****************************************************************************
...
!Returns:
    ...
    -2      cannot find correct data; equivalent to
            "missing GRIB record(s)" in original
    -3      file error -- "missing end section"
    -4      data is forecast data BUT units aren't in hours
     0	success
...
*/
{
    ...
    for (;;) {
        ...
        levels(level, PDS_KPDS6(pds), PDS_KPDS7(pds));
        if ((!strcmp(k5toa(pds), what)) && (!strcmp(level, where))) {
            retval = PDStimes(PDS_TimeRange(pds), PDS_P1(pds), PDS_P2(pds), PDS_ForecastTimeUnit(pds), frcst_val);

            ASCII_TCA_PDS_date(*frcst_val, pds, v_time, date, what);
...
*/
{
    ...
    for (j = 0; j < NTRY; j++) {
        ...
        for (i = 0; i < len; i++) {
            if (buffer[i] == 'G' && buffer[i + 1] == 'R' && buffer[i + 2] == 'I' && buffer[i + 3] == 'B' &&
                buffer[i + 7] == 1) {
                *pos = i + *pos;
...
void levels(char *x, int kpds6, int kpds7)
/*
!C**********************************************************************
!Description:
  returns a simple description of kpds6, kpds7 (level/layer data)
...
!Output Parameters:
  x   -- a simple description of kpds6, kpds7
...
*/
{
    ...
    switch (kpds6) {
        ...
        case 3:
            ...
            break;
        case 4:
            sprintf(x, "0C isotherm");
            break;
        case 5:
            sprintf(x, "cond lev");
            break;
        case 6:
            sprintf(x, "max wind lev");
            break;
        case 7:
            sprintf(x, "tropopause");
            break;
        case 8:
            sprintf(x, "nom. top");
            break;
        case 9:
            sprintf(x, "sea bottom");
            break;
        case 200:
        case 10:
            sprintf(x, "atmos col");
        ...
        case 12:
        case 212:
            sprintf(x, "low cld bot");
            break;
        case 13:
        case 213:
            sprintf(x, "low cld top");
            break;
        case 14:
        case 214:
            sprintf(x, "low cld lay");
            break;
        case 22:
        case 222:
            sprintf(x, "mid cld bot");
            break;
        case 23:
        case 223:
            sprintf(x, "mid cld top");
            break;
        case 24:
        case 224:
            sprintf(x, "mid cld lay");
            break;
        case 32:
        case 232:
            sprintf(x, "high cld bot");
            break;
        case 33:
        case 233:
            sprintf(x, "high cld top");
            break;
        case 34:
        case 234:
            sprintf(x, "high cld lay");
            break;

        case 100:
            sprintf(x, "%d mb", kpds7);
            break;
        case 101:
            sprintf(x, "%d-%d mb", o11 * 10, o12 * 10);
            break;
        case 102:
            sprintf(x, "MSL");
            break;
        case 103:
            sprintf(x, "%d m above MSL", kpds7);
            break;
        case 104:
            sprintf(x, "%d-%d m above msl", o11 * 100, o12 * 100);
            break;
        case 105:
            sprintf(x, "%d m above gnd", kpds7);
            break;
        case 106:
            sprintf(x, "%d-%d m above gnd", o11 * 100, o12 * 100);
            break;
        case 107:
            sprintf(x, "sigma=%.4f", kpds7 / 10000.0);
            break;
        case 108:
            sprintf(x, "sigma %.2f-%.2f", o11 / 100.0, o12 / 100.0);
            break;
        case 109:
            sprintf(x, "hybrid lev %d", kpds7);
            break;
        case 110:
            sprintf(x, "hybrid %d-%d", o11, o12);
            break;
        case 111:
            sprintf(x, "%d cm down", kpds7);
            break;
        case 112:
            sprintf(x, "%d-%d cm down", o11, o12);
            break;
        case 113:
            sprintf(x, "%dK", kpds7);
            break;
        case 114:
            sprintf(x, "%d-%dK", 475 - o11, 475 - o12);
            break;
        case 115:
            sprintf(x, "%d mb above gnd", kpds7);
            break;
        case 116:
            sprintf(x, "%d-%d mb above gnd", o11, o12);
            break;
        case 121:
            sprintf(x, "%d-%d mb", 1100 - o11, 1100 - o12);
            break;
        default:
            break;
    }
...
static char units[][10] = {"min\0", "hr\0", "d\0", "mon\0", "yr\0", "decade\0", "normal\0", "century\0"};

int PDStimes(int time_range, int p1, int p2, int time_unit, int *frcast_val)
/*
!C**********************************************************************
!Description:
...
!Output Parameters:
  int frcast_val -- the number of hours data is forecast for
...
*/
{
    ...
    if (time_unit >= 0 && time_unit <= 7)
        strcpy(unit, units[time_unit]);
    else
        strcpy(unit, " \0");
    ...
    /* 26-SEP-07: we need to know whether the data is
     * "anal" or "fcst" -- if it's fcst, return the
     * number of hours forcast for; if not, return zero.
     * ----------------------------------------------- */

    if (type == anal) /*printf("anl:\n");*/
        *frcast_val = 0;
    else if (type == fcst) {
        /*printf("%d%s fcst:\n",fcst_len,unit);*/ *frcast_val = fcst_len;
        if (strcmp(unit, "hr")) retval = 1; /* error condition */
    }
...
struct ParmTable parm_table_ecmwf_128[256] = {{"var0", "undefined"},                                      /* 0 */
                                              {"PRES", "Pressure [Pa]"},                                  /* 1 */
...
struct ParmTable parm_table_ecmwf_160[256] = {{"var0", "undefined"},                                      /* 0 */
                                              {"PRES", "Pressure [Pa]"},                                  /* 1 */
                                              {"PRMSL", "Pressure reduced to MSL [Pa]"},                  /* 2 */
                                              {"PTEND", "Pressure tendency [Pa/s]"},                      /* 3 */
                                              {"var4", "undefined"},                                      /* 4 */
...
struct ParmTable parm_table_omb[256] = {{"var0", "Reserved"},
                                        {"var1", "Reserved"},
                                        {"GHz6", "6.6 GHz - K"},
                                        {"GHz10", "10.7 GHz - K"},
                                        {"GHz18", "18.0 GHz - K"},
...
*/
{
    ...
    if ((user_center == -1 || center == user_center) && (user_subcenter == -1 || subcenter == user_subcenter) &&
    ...
    for (;;) {
        ...
        if ((center == -1 || center == user_center) && (subcenter == -1 || subcenter == user_subcenter) &&
            (ptable == -1 || ptable == user_ptable))
            break;
    }
...
int ASCII_TCA_PDS_date(int frcast_hours, unsigned char *pds, int v_time, char date[], char *what)
/*
!C**********************************************************************
!Description:
...
!Input Parameters:
  int frcast_hours   -- the number of hours to add for forecast data (zero for non-forecast data).
  unsigned char *pds --
...
*/
{
    ...
    if (v_time == 0) {
        /* 06-MAR-01: NOTE:  GRIB-format files have inconsistent century and year-of-century coding.
           For the year 1999, this is how year-of-century (byte 20) and century (byte 32) are encoded:

                         byte   20   32
            file
           -------------------------------
            GDAS_0ZF            99   20     "The ninty-ninth year of the twentieth century"
            OZ_DAILY            99   20     same
           ...
           For the year 2000:
            ...
            file
           -------------------------------
            GDAS_0ZF           100   20     "The hundreth year of the twentieth century"
            OZ_DAILY             0   20     inconsistent
           ...
           This is for the year 2001:
            ...
            file
           -------------------------------
            GDAS_0ZF             1   21     "The first year of the twenty-first century"
            OZ_DAILY             1   20     inconsistent

           For now I'm assuming the GDAS_0ZF files are correct, and I'm subtracting 1 from
           the century, multiplying by 100, and adding YOC; for OZ_DAILY files we
           simply add 100.  But this won't work for OZ_DAILY files from before 2000.
         */

        if ((strstr(what, "OZ")) || (strstr(what, "oz")))
            year = PDS_Year4(pds) + 100;
        else
            year = PDS_Year4(pds);

        /* 21-JUL-01:
           The MOSS-4 test shows us that, of course, we have to read OZ_DAILY files
           from before 2000.  So here is the strategy:
                 take year-of-century from byte 20 (12, here), ignore century (byte 32).
                 if YOC is >= 100, subtract 100.
                 if YOC is > 80, add 1900; else add 2000.
           This should work for all NCEP files from 1980 until 2080.
           --- JPR, SSAI. */
        ...
        if (frcast_hours > 0) addd_time(&year, &month, &day, &hour, frcast_hours, 1);
...
*/
{
    ...
    /*fprintf(stderr,"addd_time: undefined time unit %d\n", unit);*/
    return 1;
}
...
*/
{
    ...
    *hour = PDS_Hour(pds);
    ...
    return addd_time(year, month, day, hour, dtime, unit);
}
